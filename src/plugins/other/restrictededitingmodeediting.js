...
import {
  getMarkerAtPosition,
  isSelectionInMarker,
} from "./restrictededitingmode/utils";
...
export default class RestrictedEditingModeEditing extends Plugin {
  ...
  init() {
    ...
    const allowedCommands = editor.config.get(
      "restrictedEditing.allowedCommands"
    );

    allowedCommands.forEach((commandName) =>
      this._allowedInException.add(commandName)
    );
    ...
    editor.commands.add(
      ...
      new RestrictedEditingNavigationCommand(editor, "backward")
    );
    //绑定指令 跳转到下一个可编辑区域
    editor.commands.add(
      "goToNextRestrictedEditingException",
      new RestrictedEditingNavigationCommand(editor, "forward")
    );
    // 绑定一些HotKey tab：跳转到下一个可编辑区域
    editor.keystrokes.set(
      "Tab",
      getCommandExecuter(editor, "goToNextRestrictedEditingException")
    );
    // 绑定一些HotKey shift+tab：跳转到上一个可编辑区域
    editor.keystrokes.set(
      "Shift+Tab",
      getCommandExecuter(editor, "goToPreviousRestrictedEditingException")
    );
    // 绑定一些HotKey ctrl+A：全选

    editor.keystrokes.set("Ctrl+A", getSelectAllHandler(editor));
    //可修改试视图的 callBack
    editingView.change((writer) => {
      for (const root of editingView.document.roots) {
        //给root 添加 限制编辑模式的class
  ...
  _setupRestrictions() {
    ...
    this.listenTo(
      ...
      (evt, data) => {
        if (
          data.method == "cut" &&
          !isRangeInsideSingleMarker(editor, selection.getFirstRange())
        ) {
          evt.stop();
        }
      },
    ...
    const allowedAttributes = editor.config.get(
      "restrictedEditing.allowedAttributes"
    );
    model.schema.addAttributeCheck(
      onlyAllowAttributesFromList(allowedAttributes)
    );
  ...
  _enableCommands(marker) {
    ...
    for (const [commandName, command] of editor.commands) {
      ...
      if (
        isDeleteCommandOnMarkerBoundaries(
          commandName,
...
function getSelectAllHandler(editor) {
  return (data, cancel) => {
    ...
    if (
      markerRange.containsRange(selectionRange, true) ||
...
function isDeleteCommandOnMarkerBoundaries(
  commandName,
  selection,
  markerRange
) {
  ...
  if (
    ...
    selection.isCollapsed &&
    markerRange.end.isEqual(selection.focus)
  ) {
    return true;
  }
...
function restrictDeleteContent(editor) {
  return (evt, args) => {
    ...
    const marker =
      getMarkerAtPosition(editor, selection.focus) ||
      getMarkerAtPosition(editor, selection.anchor);
    ...
    const allowedToDelete = marker
      .getRange()
      .getIntersection(selection.getFirstRange());
  ...
  return (writer) => {
    ...
    for (const {
      ...
      if (!oldRange && !newRange.isFlat) {
        ...
        const fixedStart = startIsHigherInTree
          ? newRange.start
          : writer.createPositionAt(end.parent, 0);
        const fixedEnd = startIsHigherInTree
          ? writer.createPositionAt(start.parent, "end")
          : newRange.end;
