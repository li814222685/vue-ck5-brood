...
import { TABLE_ANCHOR_TOOLBAR, TABLE_ANCHOR_DEL, TABLE_ANCHOR_ROW_BELOW } from "./constant";
...
export default class TableAnchorToolbar extends Plugin {
  ...
  getTableWidgetAncestor(selection, tableUtils) {
    ...
    const isTableAnchorElement = blocks => {
      ...
      try {
        const { row, column } = tableUtils.getCellLocation(firstEleParent);
  ...
  afterInit() {
    ...
    widgetToolbarRepository.register(TABLE_ANCHOR_TOOLBAR, {
      items: [TABLE_ANCHOR_TOOLBAR, TABLE_ANCHOR_ROW_BELOW, TABLE_ANCHOR_DEL],
